...
class TicTacToe() {
    ...
    private var dificultad= Difficulty.FACIL
   ...
   fun getPersonSymbol(): String {
    ...
    public fun setComputerMove(): Int{
        ...
        if(dificultad==Difficulty.EXPERTO) {
            for (i in 0..8) {
        ...
        if(dificultad==Difficulty.DIFICIL||dificultad==Difficulty.EXPERTO) {
            for (i in 0..8) {
                if (this.board[i] !== personSymbol && this.board[i] !== robotSymbol) {
